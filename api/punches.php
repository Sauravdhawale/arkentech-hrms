...
try{
 ...
 $pdo=db();$q=$pdo->query("SELECT data FROM hr_records WHERE module='devices' AND status='Enabled'");$enabled=false;foreach($q as $r)if((json_decode($r['data'],true)['device_code']??'')===$device)$enabled=true;if(!$enabled)answer(403,['error'=>'Device is not enabled']);
