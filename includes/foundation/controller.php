...
if($_SERVER['REQUEST_METHOD']==='POST'){
 ...
 try{
  if(str_starts_with($action,'att_')){$notice=att_handle_post($pdo,$user,$action,$_POST,$_FILES);
   ...
   if($action==='bulk_employees'){
    $notice=foundation_bulk_employees($pdo,$user,$_POST);
    ...
    need($pdo,$user,$action==='delete_employee'?'employees.delete':'employees.edit');if($action==='delete_employee'&&$user['role']!=='super_admin')throw new InvalidArgumentException('Only Super Admin can delete employees.');$id=(int)($_POST['id']??0);$pdo->beginTransaction();$q=$pdo->prepare("SELECT u.*,e.employment_status FROM users u JOIN employees e ON e.user_id=u.id WHERE u.id=? AND e.deleted_at IS NULL FOR UPDATE");$q->execute([$id]);$u=$q->fetch(PDO::FETCH_ASSOC);
