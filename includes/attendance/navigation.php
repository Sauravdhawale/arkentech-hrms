...
if($visible):?><details class="attendance-navigation" <?=in_array($page,array_merge(array_keys($attendanceLinks),['attendance','attendance_history','attendance_requests']),true)?'open':''?>><summary>◷ <span>Attendance</span></summary><div><?php foreach($visible as $key=>$label)fnav($key,$label);?></div></details><?php endif;
